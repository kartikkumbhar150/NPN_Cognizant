...
class PromptPackage(StrictModel):
    ...
    ``fact_values`` is a safe structured view of the approved grounded facts
    (``fact_id -> value``). It lets providers deterministically reference fact
    values without parsing rendered prompt text. It contains product facts
    only — never customer identifiers or propensity.
    """
